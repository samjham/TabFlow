...
- **Cross-browser sync Phase 4 (re-enable push-on-snapshot + pull-on-resume) is deferred.** The deterministic-ID migration (Phases 1–3) landed on 2026-04-15 — tab IDs are now content-derived `tab-<16hex>` hashes (see `packages/browser-extension/src/utils/tabId.ts`), so Chrome and Firefox produce the same ID for the same URL in the same workspace, which solves the original collision. However, `snapshotActiveWorkspace` still doesn't push to Supabase and `CLAIM_ACTIVE_DEVICE` still doesn't pull — both were disabled during the rollback and haven't been turned back on. Before re-enabling, stress-test locally that (a) the new `buildMainWindowUrlIndex` lookup path handles edge cases like duplicate URLs and suspended-tab wrappers correctly, (b) `pullAll` doesn't mass-import duplicates now that IDs are stable, and (c) realtime echo from push-on-snapshot doesn't cause UI thrash even with the stable IDs. Chrome is currently local-only for day-to-day tab activity; only explicit actions (`CREATE_WORKSPACE`, rename, delete, color change) push.
