...
- **Cross-browser sync Phase 4 redesigned after the first attempt corrupted Chrome via Firefox.** The original Phase 4 (pushed every snapshot to Supabase) broke because `snapshotActiveWorkspace` scans the browser window's actual open tabs and writes them to the active workspace. On a freshly-loaded second browser, the window has random tabs (about:debugging, etc.) that have nothing to do with the pulled active workspace, so the snapshot overwrote the cloud with garbage and Chrome picked it up via realtime. Push-on-snapshot is now permanently disabled. `CLAIM_ACTIVE_DEVICE` does a full materialization flow (pull → close current tabs → open workspace's tabs → re-snapshot → verify → claim) and a new `safeToPush` gate prevents push until a successful claim completes. Phase 4 needs fresh cross-browser testing with the new design before 0.1.2 ships — see the 2026-04-15 changelog entry for the architectural details.
