...
- **Cross-browser sync Phase 4 re-enabled on 2026-04-15 and needs local stress-testing before publish.** Push-on-snapshot and pull-on-claim are both live now that tab IDs are deterministic (`tab-<16hex>` content hashes) and can't collide between Chrome and Firefox. Sam is testing Chrome ↔ Firefox round-trips locally before pushing 0.1.2 to the Chrome Web Store. Still worth watching during that test: (a) duplicate URLs across workspaces, (b) `pullAll` not mass-importing duplicates, (c) realtime echo from push-on-snapshot not thrashing the UI, (d) suspended-tab wrappers (`suspended.html?url=…`) surviving push/pull correctly.
