...
export class MessageHandler {
  ...
  private async handleToggleTabPersistent(payload: any): Promise<Response> {
    const { tabId, persistent, workspaceId } = payload || {};
    if (!tabId) return { success: false, error: 'tabId is required' };
    ...
    // 0.1.53: Fast path. The UI passes workspaceId (the currently-active
    // workspace, since the pushpin is only visible for tabs in the active
    // workspace's tile grid). Direct lookup avoids iterating all N
    // workspaces which took ~1s in Sam's 19-workspace install and opened
    // a wide race window against saveCurrentTabsToWorkspace — the pushpin
    // toggle arrived too late and the snapshot's stale in-memory copy
    // clobbered persistent=true back to false. PIP tab then got closed.
    if (workspaceId && typeof workspaceId === 'string') {
      const wsTabs = await this.storage.getTabs(workspaceId);
      target = wsTabs.find((t) => t.id === tabId) ?? null;
    }

    // Fallback: no workspaceId in payload, or the tab wasn't in that
    // workspace. Iterate all workspaces (backward compat).
    if (!target) {
      const workspaces = await this.storage.getWorkspaces(LOCAL_USER_ID);
      for (const ws of workspaces) {
        const wsTabs = await this.storage.getTabs(ws.id);
