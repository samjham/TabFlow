...
async function initializeSync(userId: string) {
  try {
    ...
    const existingSalt = stored.encryptionSalt
      ? new Uint8Array(JSON.parse(stored.encryptionSalt))
      : undefined;

    const { key, salt } = await deriveKey(stored.encryptionPassphrase, existingSalt);

    // Store salt for future sessions (if new)
  ...
  try {
    ...
    if (!keeper) {
      ...
      if (stored.tabFlowTabId) {
        keeper = allTabs.find((t) => t.id === stored.tabFlowTabId) || null;
        if (keeper) console.log(`[TabFlow] Found TabFlow tab by stored ID (tab ${keeper.id})`);
      }
    }
