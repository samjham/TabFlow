...
export const SetupWizard: React.FC<SetupWizardProps> = ({ onComplete, onSkip }) => {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const response = await fetch(`${urlTrimmed}/rest/v1/`, {
        headers: {
      ...
      if (!response.ok) {
        setError(`Connection failed (HTTP ${response.status}). Double-check your URL and anon key.`);
        setTesting(false);
        return;
      }
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        {step === 'enter-credentials' && (
          <>
            <h1 style={styles.heading}>Connect Your Project</h1>
            <p style={styles.body}>
              In your Supabase dashboard, go to{' '}
              <strong style={{ color: '#e0e6ed' }}>Project Settings</strong> (gear icon at the
              bottom of the left sidebar) → <strong style={{ color: '#e0e6ed' }}>API</strong>.
              Copy the two values below.
            </p>
            ...
            <div style={styles.formGroup}>
              ...
              />
              <span style={styles.hint}>Listed as "Project URL" in the API settings</span>
            </div>
            ...
            <div style={styles.formGroup}>
              ...
              <input
                ...
                onBlur={(e) => (e.currentTarget.style.borderColor = '#3a3f4b')}
              />
              <span style={styles.hint}>Listed as "anon public" under Project API keys</span>
            </div>
