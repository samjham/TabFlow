...
export const NewTab: React.FC<NewTabProps> = ({ user, onSignOut }) => {
  ...
  useEffect(() => {
    ...
    // Listen for device status changes via storage.session
    const listener = (
      changes: { [key: string]: chrome.storage.StorageChange },
      areaName: string
    ) => {
      if (areaName !== 'session' || !changes.deviceStatus) return;
  ...
  const handleDrop = async (targetWorkspaceId: string) => {
    ...
    currentOrder.splice(targetIndex, 0, draggedWorkspaceId);
      ...
      >
        ...
        <div style={styles.spacesSection}>
          ...
          <div
            ...
            onDragOver={(e) => {
              // Allow dropping in the empty space below the last workspace
              if (draggedWorkspaceId) {
                e.preventDefault();
                setDragOverWorkspaceId('__bottom__');
              }
            }}
            onDrop={async (e) => {
              e.preventDefault();
              if (!draggedWorkspaceId) return;
              // Move dragged workspace to the very end
              const currentOrder = workspaces.map((ws) => ws.id);
              const draggedIndex = currentOrder.indexOf(draggedWorkspaceId);
              if (draggedIndex === currentOrder.length - 1) {
              ...
              currentOrder.splice(draggedIndex, 1);
              currentOrder.push(draggedWorkspaceId);
              setDraggedWorkspaceId(null);
              setDragOverWorkspaceId(null);
              try {
                await reorderWorkspaces(currentOrder);
  ...
  return (
    <div
      ...
      style={{
        ...
        borderTopWidth: isDragOver ? '2px' : '0px',
