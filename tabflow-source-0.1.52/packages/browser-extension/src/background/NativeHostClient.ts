...
interface NativeMessage {
  ...
  excludeTitle?: string;
  /**
   * Optional process name for hideMinimized — "chrome" (default) or "firefox".
   * Tells the native host which browser's processes to walk when looking for
   * minimized windows to hide. Older host binaries (pre-2026-04-23) ignore
   * this and always target Chrome.
   */
...
export class NativeHostClient {
  ...
  /**
   * Hides all MINIMIZED browser windows from the taskbar.
   * This is the safest approach — only minimized windows (TabFlow's
   * hidden workspace windows) are affected. The user's main browser
   * window is never touched because it's not minimized.
   *
   * @param processName "chrome" or "firefox" — the browser whose
   *   minimized windows should be hidden. Defaults to "chrome" so old
   *   host binaries that ignore the param keep working unchanged.
   */
  async hideMinimized(processName: 'chrome' | 'firefox' = 'chrome'): Promise<number> {
    const response = await this.sendMessage({
    ...
    if (response.success) {
      console.log(`[TabFlow] Hidden ${response.hiddenCount} minimized ${processName} windows from taskbar`);
      return response.hiddenCount || 0;
    } else {
      console.warn(`[TabFlow] Failed to hide minimized ${processName} windows: ${response.error}`);
      return 0;
    }
  }
