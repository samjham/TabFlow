...
export async function initialize(): Promise<SupabaseClient> {
  ...
  supabaseClient = createClient(url, anonKey, {
    ...
    global: {
      // Force all Supabase REST requests to bypass HTTP cache. Without this,
      // Chrome's service worker can return a stale cached response for
      // checkActiveDevice after waking from sleep, causing it to see its own
      // device_id instead of the current one (set by Firefox) and auto-claim.
