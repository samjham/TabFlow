...
class IndexedDBStorageAdapter implements StorageAdapter {
   ...
   *
   * 0.1.50: Enforces the single-active-workspace invariant. If the
   * incoming workspace has isActive=true, this method atomically clears
   * isActive on ALL OTHER workspaces belonging to the same user, in a
   * single Dexie transaction, then saves the target. This prevents the
   * multi-active-workspace corruption Sam hit in the 0.1.49 diagnostic
   * (two workspaces both flagged isActive=true, causing the reconciler
   * to see an ambiguous state).
   *
  ...
  async saveWorkspace(workspace: Workspace): Promise<void> {
    if (workspace.isActive) {
      // Atomic: single transaction covers the clear + the put. If anything
      // throws, Dexie rolls the whole thing back so we can't leave the DB
      // in a state with zero active workspaces.
      await this.db.transaction('rw', this.db.workspaces, async () => {
        const siblings = await this.db.workspaces
          .where('userId')
          .equals(workspace.userId)
          .toArray();
        for (const w of siblings) {
          if (w.id !== workspace.id && w.isActive) {
            await this.db.workspaces.put({ ...w, isActive: false });
          }
        }
  ...
  /**
   * Clears the entire workspace history table.
   * Used by the destructive pull on Resume Working Here, so the cloud's
   * history becomes the only source of truth on this device.
   */
  async clearWorkspaceHistory(): Promise<number> {
    const count = await this.db.workspaceHistory.count();
    await this.db.workspaceHistory.clear();
    return count;
  }
   ...
   */
  async saveThumbnail(tabId: string, dataUrl: string): Promise<void> {
    await this.db.thumbnails.put({ url: tabId, dataUrl, capturedAt: Date.now() });
  }
   ...
   */
  async getThumbnails(tabIds: string[]): Promise<Record<string, string>> {
    const entries = await this.db.thumbnails.where('url').anyOf(tabIds).toArray();
  ...
  /**
   * Clears the entire deletedWorkspaces archive.
   * Used by the destructive pull on Resume Working Here.
   */
  async clearDeletedWorkspaces(): Promise<number> {
    const count = await this.db.deletedWorkspaces.count();
    await this.db.deletedWorkspaces.clear();
    return count;
  }

  /**
   * Look up an archived workspace by its id. Used by sync paths that
   * want to detect already-pushed entries without re-reading the full set.
   */
  async getDeletedWorkspaceById(id: string): Promise<DeletedWorkspace | null> {
    const entry = await this.db.deletedWorkspaces.get(id);
    return entry || null;
  }
