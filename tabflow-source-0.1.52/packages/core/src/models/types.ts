...
export interface Tab {
  ...
  updatedAt: Date;
  /** Saved horizontal scroll position (pixels from left) */
  scrollX?: number;
  /** Saved vertical scroll position (pixels from top) */
  scrollY?: number;
  ...
  createdAt?: Date;
  /** When true, this tab is preserved across workspace switches by using
   * Firefox's `chrome.tabs.hide()` API instead of being closed. Preserves
   * PIP, form state, audio, etc. Firefox-only — on Chrome the checkbox
   * is disabled since `chrome.tabs.hide()` doesn't exist there.
   * Default false (opt-in). Synced through Supabase. */
  persistent?: boolean;
}
